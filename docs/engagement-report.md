...
- **אימות:** משוחזר על **v3.0.24** ועל **master (SHA 69b7aea)**; שני קלטים עצמאיים. ⚠️ **הגרסה היציבה האחרונה (סדרת 3.2.x) טרם נבדקה** — שחזור על master אינו מוכיח שהבנייה/הנתיב של ה-stable הנוכחי מושפעים; יש לאמת על ה-tag היציב האחרון לפני disclosure (הקוד `parsej2k_imp` זהה על master, אך יש לוודא build).
